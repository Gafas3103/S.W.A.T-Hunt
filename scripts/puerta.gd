...
@export var velocidad_candado := 3.0

@onready var bisagra: Node3D = $Bisagra
...
func interactuar(jugador: Node) -> void:
	...
	giro.tween_property(bisagra, "rotation_degrees:y", angulo_abierta, duracion_apertura)
	await giro.finished
	abierta = true
	abriendo = false
	...
	if jugador.has_method("mostrar_aviso"):
		jugador.mostrar_aviso("Puerta abierta")
